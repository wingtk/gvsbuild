...
class Builder:
    ...
    def __check_vs(self, opts):
        ...
        if opts.vs_install_path:
            vs_paths = {opts.vs_install_path: None}
        ...
        output = None
        log.message("")
        log.message("Checking Visual Studio installation(s):")
        for path, name in vs_paths.items():
            formatted_name = f"{name} @ {path}" if name else path
            try:
                output = self.__check_vs_install(opts, path, assert_vs_version)
            except CheckVsInstallError as check_vs_error:
                log.message(f" [{formatted_name}]: Skipping because {check_vs_error}")

            if output is not None:
                log.message(
                    f" [{formatted_name}]: Validation passed, using this one :)"
                )
                break
