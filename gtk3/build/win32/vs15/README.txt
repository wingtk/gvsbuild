...
build/win32/vs14/README.txt file in glib for details where to unpack them.
...
GTK+ stack, VS14 project files are either available under
$(srcroot)/build/vs14 in the case of GLib (stable/unstable), ATK
(stable/unstable) and GDK-Pixbuf (unstable), and should be in the next
unstable version of Pango.  There is no known official VS14 build
...
<root>\vs14\<PlatformName>\lib and GTK+ headers into
<root>\vs14\<PlatformName>\include\gtk-3.0. This is then from where
...
**:Put the sources of the packages marked with ** in <root>\<package-
    source-tree>, and build with VS14 from there.
