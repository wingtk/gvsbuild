...
$items = @{
	...
	'cairo' = @{
		'ArchiveUrl' = 'http://cairographics.org/snapshots/cairo-1.15.2.tar.xz'
	...
	'openssl' = @{
		'ArchiveUrl' = 'ftp://ftp.openssl.org/source/openssl-1.0.2f.tar.gz'
		'Dependencies' = @()
	};
...
$items['librsvg'].BuildScript = {
	...
	Exec msbuild build\win32\vs$VSVer\librsvg.sln /p:Platform=$platform /p:Configuration=Release /maxcpucount /nodeReuse:True
